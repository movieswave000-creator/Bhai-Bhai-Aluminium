...
function renderProducts(){
  ...
  el.innerHTML=(data.products||[]).map(p=>`<div class="adminItem"><form class="editProduct" data-id="${esc(p.id)}" enctype="multipart/form-data"><div class="itemTop"><b>${esc(p.name)}</b><button type="button" class="danger" onclick="removeProduct('${esc(p.id)}')">Delete</button></div><div class="formGrid"><input name="name" value="${esc(p.name)}" required><input name="bn" value="${esc(p.bn||'')}" placeholder="বাংলা"><input name="hi" value="${esc(p.hi||'')}" placeholder="हिन्दी"><input name="category" value="${esc(p.category||'')}" placeholder="Category"><input name="size" value="${esc(p.size||'')}" placeholder="Size"><input name="price" value="${esc(p.price||'')}" placeholder="Price"><input name="offer" value="${esc(p.offer||'')}" placeholder="Offer"><select name="stock"><option ${p.stock==='In Stock'?'selected':''}>In Stock</option><option ${p.stock==='Out of Stock'?'selected':''}>Out of Stock</option></select><input name="image" type="file" accept="image/*"><textarea name="description">${esc(p.description||'')}</textarea><button class="primary" type="submit">Save Product</button></div></form></div>`).join('')||'<p class="muted">No products yet.</p>';
  document.querySelectorAll('.editProduct').forEach(form=>form.onsubmit=async e=>{e.preventDefault();try{await api('/api/admin/product/'+encodeURIComponent(form.dataset.id),{method:'PUT',body:new FormData(form)});await load();}catch(x){alert(x.message);}});
}
...
$('#product').onsubmit=async e=>{e.preventDefault();try{await api('/api/admin/product',{method:'POST',body:new FormData(e.target)});e.target.reset();await load();}catch(x){alert(x.message);}};
