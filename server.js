...
app.post('/api/admin/product',auth,upload.single('image'),(req,res)=>{try{let d=read();d.products.unshift({id:Date.now().toString(),name:req.body.name||'Product',bn:req.body.bn||'',hi:req.body.hi||'',category:req.body.category||'Other',size:req.body.size||'',price:req.body.price||'',offer:req.body.offer||'',stock:req.body.stock||'In Stock',image:req.file?'/uploads/'+req.file.filename:(req.body.image||''),description:req.body.description||''});write(d);res.json({ok:true});}catch(e){console.error(e);res.status(500).json({error:'Could not save product.'});}});
app.put('/api/admin/product/:id',auth,upload.single('image'),(req,res)=>{try{let d=read(),p=d.products.find(x=>x.id===req.params.id);if(!p)return res.status(404).json({error:'Product not found.'});Object.assign(p,{name:req.body.name??p.name,bn:req.body.bn??p.bn,hi:req.body.hi??p.hi,category:req.body.category??p.category,size:req.body.size??p.size,price:req.body.price??p.price,offer:req.body.offer??p.offer,stock:req.body.stock??p.stock,description:req.body.description??p.description});if(req.file)p.image='/uploads/'+req.file.filename;else if(req.body.image!==undefined)p.image=req.body.image;write(d);res.json({ok:true});}catch(e){console.error(e);res.status(500).json({error:'Could not save product.'});}});
