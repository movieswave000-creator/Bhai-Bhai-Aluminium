...
function write(d){
  // Keep the store write cross-platform. fs.renameSync(tmp, DATA) can fail on
  // Windows when DATA already exists, which made the admin save action look
  // like a crash. The JSON is small, so a direct UTF-8 write is reliable.
  const payload=JSON.stringify(d,null,2);
  fs.writeFileSync(DATA,payload,'utf8');
  return true;
}
function maskEmail(e){if(!e)return '';const [u,dom]=e.split('@');return (u?u[0]+'***':'***')+'@'+dom;}
function cleanUsername(v){return String(v||'').trim();}
function normalizeSizes(body, fallback){
  const sizes=[];
  const src=Array.isArray(body.sizes)?body.sizes:null;
  if(src){ for(const x of src){ const size=String(x?.size??'').trim(), price=String(x?.price??'').trim(), offer=String(x?.offer??'').trim(); if(size||price||offer) sizes.push({size,price,offer}); } }
  else {
    const keys=Object.keys(body||{}).filter(k=>/^size_\d+$/.test(k)).sort((a,b)=>Number(a.split('_')[1])-Number(b.split('_')[1]));
    for(const k of keys){const i=k.split('_')[1],size=String(body[k]??'').trim(),price=String(body['price_'+i]??'').trim(),offer=String(body['offer_'+i]??'').trim();if(size||price||offer)sizes.push({size,price,offer});}
  }
  if(!sizes.length && fallback && (fallback.size||fallback.price||fallback.offer)) return [{size:String(fallback.size||''),price:String(fallback.price||''),offer:String(fallback.offer||'')}];
  return sizes;
}

app.disable('x-powered-by');
// Render/Hostinger may sit behind a reverse proxy. Trust the proxy so secure
// session cookies work correctly over HTTPS; without this, admin API calls can
// appear as Unauthorized immediately after a successful login.
app.set('trust proxy', 1);
app.use(express.json({limit:'2mb'}));
app.use(express.urlencoded({extended:true}));
app.use(session({secret:process.env.SESSION_SECRET||'CHANGE_THIS_SESSION_SECRET_IN_RENDER',resave:false,saveUninitialized:false,cookie:{httpOnly:true,sameSite:'lax',secure:'auto',maxAge:86400000}}));
app.use(express.static(path.join(__dirname,'public')));
function auth(req,res,next){if(req.session.admin)return next();res.set('Cache-Control','no-store');res.status(401).json({error:'Unauthorized'});}
...
app.post('/api/admin/product',auth,upload.single('image'),(req,res)=>{try{let d=read();const sizes=normalizeSizes(req.body);const first=sizes[0]||{};d.products.unshift({id:Date.now().toString(),name:req.body.name||'Product',bn:req.body.bn||'',hi:req.body.hi||'',category:req.body.category||'Other',sizes,size:first.size||'',price:first.price||'',offer:first.offer||'',stock:req.body.stock||'In Stock',image:req.file?'/uploads/'+req.file.filename:'',description:req.body.description||''});write(d);res.json({ok:true,product:d.products[0]});}catch(e){console.error(e);res.status(500).json({error:'Could not save product.'});}});
app.put('/api/admin/product/:id',auth,upload.single('image'),(req,res)=>{try{let d=read(),p=d.products.find(x=>x.id===req.params.id);if(!p)return res.status(404).json({error:'Product not found.'});const sizes=normalizeSizes(req.body,p);const first=sizes[0]||{};Object.assign(p,{name:req.body.name??p.name,bn:req.body.bn??p.bn,hi:req.body.hi??p.hi,category:req.body.category??p.category,sizes,size:first.size||'',price:first.price||'',offer:first.offer||'',stock:req.body.stock??p.stock,description:req.body.description??p.description});if(req.file)p.image='/uploads/'+req.file.filename;write(d);res.json({ok:true,product:p});}catch(e){console.error(e);res.status(500).json({error:'Could not save product.'});}});
...
app.put('/api/admin/settings',auth,upload.single('logoFile'),(req,res)=>{try{let d=read();d.settings={...d.settings,...req.body};if(req.file)d.settings.logo='/uploads/'+req.file.filename;write(d);res.json({ok:true,settings:d.settings});}catch(e){console.error(e);res.status(500).json({error:'Could not save website settings.'});}});
