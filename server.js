...
function parseSizes(body){
 ...
 if(raw){try{const x=JSON.parse(raw); if(Array.isArray(x)) for(const v of x){const size=String(v.size??'').trim(); if(size) sizes.push({size,price:String(v.price??'').trim(),offer:String(v.offer??'').trim()});}}catch{}}
 if(!sizes.length && String(body.size||'').trim()) sizes.push({size:String(body.size).trim(),price:String(body.price||'').trim(),offer:String(body.offer||'').trim()});
 return sizes;
}
app.post('/api/admin/product',auth,upload.single('image'),(req,res)=>{try{let d=read();const sizes=parseSizes(req.body);d.products.unshift({id:crypto.randomUUID(),name:req.body.name||'Product',bn:req.body.bn||'',hi:req.body.hi||'',category:req.body.category||'Other',sizes,size:sizes[0]?.size||'',price:sizes[0]?.price||'',offer:sizes[0]?.offer||'',stock:req.body.stock||'In Stock',image:req.file?'/uploads/'+req.file.filename:(req.body.image||''),description:req.body.description||''});write(d);res.json({ok:true});}catch(e){console.error(e);res.status(500).json({error:e.message||'Could not save product.'});}});
app.put('/api/admin/product/:id',auth,upload.single('image'),(req,res)=>{try{let d=read(),p=d.products.find(x=>x.id===req.params.id);if(!p)return res.status(404).json({error:'Product not found.'});const sizes=parseSizes(req.body);Object.assign(p,{name:req.body.name??p.name,bn:req.body.bn??p.bn,hi:req.body.hi??p.hi,category:req.body.category??p.category,sizes:sizes.length?sizes:(Array.isArray(p.sizes)?p.sizes:[]),size:sizes[0]?.size??p.size??'',price:sizes[0]?.price??p.price??'',offer:sizes[0]?.offer??p.offer??'',stock:req.body.stock??p.stock,description:req.body.description??p.description});if(req.file)p.image='/uploads/'+req.file.filename;write(d);res.json({ok:true,product:p});}catch(e){console.error(e);res.status(500).json({error:e.message||'Could not save product.'});}});
